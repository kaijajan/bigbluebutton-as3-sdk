...
//                trace("error: " + response.message);
			...
			if (response.returncode == "SUCCESS")
			{
				...
				else if (config.role == Role.MODERATOR)
				{
					...
					api.create(
							...
							0,
							meta,
							"fd.tt.gzedu.com/demo/pdfs/presentation2.pdf");
				}
				else
				{
		...
		{
			uploading = false;
			trace('bbb: upload io error ' + e.text);
		}

		private function onSecurityError(e:SecurityErrorEvent):void
		{
			uploading = false;
			trace('bbb: upload security error ' + e.text);
		}
